<script lang="ts">
	...
	import LoadingMark from "./loading-mark.svelte";
...
<div class="absolute inset-0 overflow-hidden bg-black">
  ...
  {/if}
  <div
    class="absolute inset-0 bg-linear-to-t from-black via-black/40 to-black/60"
  ></div>
  ...
  {#if certification || genres.length > 0}
    ...
    >
      {#if certification}
        <div
          class="rounded-md border-2 border-white/70 px-3 py-1 text-lg font-bold tracking-wider text-white"
        >
          {certification}
        </div>
      {/if}
