...
>
  <TopBar {title} {subheading} {hasInfo} {infoOpen} {onToggleInfo} {onBack} {onSources} />
  ...
  {#if !transport.loading && !fatalError && !infoOpen && !subtitlesOpen}
    <CenterControls {transport} onSeek={player.seek} onTogglePlay={player.togglePlay} />
  {/if}

  <!-- Bottom bar -->
  <div class="pointer-events-auto flex flex-col gap-2 px-3 pb-3 text-white sm:px-4 sm:pb-4">
    <ScrubBar {transport} {bufferedRatio} {progressRatio} onScrub={player.onScrub} />
    <ControlRow
      {transport}
      {player}
      {media}
