...
export const NUVIO_PUBLISHABLE_KEY = 'sb_publishable_1Clq8rlTVACkdcZuqr6_AD__xUUC_EN';
...
export class NuvioApiError extends Error {
	...
	constructor(status: number, body: NuvioApiErrorBody, fallbackMessage: string) {
	...
	static async fromResponse(response: Response): Promise<NuvioApiError> {
		...
		return new NuvioApiError(response.status, body, `Nuvio request failed with status ${response.status}`);
	}
}
...
export class NuvioClient {
	...
	async rpc<Name extends RpcName>(
		fn: Name,
		...args: {} extends RpcParams<Name> ? [params?: RpcParams<Name>] : [params: RpcParams<Name>]
	...
	async signInWithPassword(credentials: EmailPasswordCredentials): Promise<AuthSession> {
		return this.authenticate(`${this.baseUrl}/auth/v1/token?grant_type=password`, credentials);
	}

	async refreshSession(refreshToken?: string): Promise<AuthSession> {
		const token = refreshToken ?? this.currentSession?.refresh_token;
		if (!token) {
			throw new NuvioApiError(400, { message: 'No refresh token available' }, 'No refresh token available');
		}
		return this.authenticate(`${this.baseUrl}/auth/v1/token?grant_type=refresh_token`, {
			refresh_token: token
		});
	}
	...
	async signOut(): Promise<void> {
		if (this.currentSession) {
			await this.http<void>(`${this.baseUrl}/auth/v1/logout`, { method: 'POST' });
		}
		this.setSession(null);
	}

	async getUser(): Promise<NuvioUser> {
		return this.http<NuvioUser>(`${this.baseUrl}/auth/v1/user`, { method: 'GET' });
	}
	...
	readonly profiles = {
		...
		replace: (params: PushProfilesParams): Promise<void> => this.rpc('sync_push_profiles', params),
	...
	readonly addons = {
		list: (profileId: number): Promise<Addon[]> => {
			...
			return this.http<Addon[]>(`${this.baseUrl}/rest/v1/addons?${query}`, { method: 'GET' });
		},
		replace: (params: PushAddonsParams): Promise<void> => this.rpc('sync_push_addons', params)
	};
	...
	readonly library = {
		...
		pullDelta: (params: PullLibraryDeltaParams = {}): Promise<LibraryDeltaEvent[]> =>
		...
		replaceLegacy: (params: PushLibraryParams): Promise<void> => this.rpc('sync_push_library', params)
	};
	...
	readonly watchProgress = {
		...
		pullDelta: (params: PullWatchProgressDeltaParams = {}): Promise<WatchProgressDeltaEvent[]> =>
		...
		delete: (progressKey: string, profileId?: number): Promise<void> =>
			this.rpc('sync_delete_watch_progress', { p_progress_key: progressKey, ...profileScope(profileId) }),
		deleteMany: (progressKeys: string[], profileId?: number): Promise<void> =>
			this.rpc('sync_delete_watch_progress', { p_keys: progressKeys, ...profileScope(profileId) })
	};
	...
	readonly watchHistory = {
		...
		pullDelta: (params: PullWatchedItemsDeltaParams = {}): Promise<WatchedItemDeltaEvent[]> =>
		...
		delete: (keys: WatchedItemKey[], profileId?: number): Promise<void> =>
			this.rpc('sync_delete_watched_items', { p_keys: keys, ...profileScope(profileId) })
	};
	...
	readonly homeCatalog = {
		pull: (params: PullHomeCatalogSettingsParams): Promise<HomeCatalogSettings[]> =>
	...
	readonly collections = {
		pull: (profileId: number): Promise<CollectionsBlob[]> =>
			this.rpc('sync_pull_collections', { p_profile_id: profileId } satisfies PullCollectionsParams),
		replace: (params: PushCollectionsParams): Promise<void> => this.rpc('sync_push_collections', params)
	};
	...
	async healthCheck(): Promise<HealthCheck> {
		const response = await this.fetchImplementation(`${this.baseUrl}/functions/v1/health-check`);
		if (!response.ok) {
	...
	async getSupporterWall(query: SupporterWallQuery = {}): Promise<SupporterWall> {
		const search = new URLSearchParams();
		if (query.limit !== undefined) search.set('limit', String(query.limit));
		if (query.offset !== undefined) search.set('offset', String(query.offset));
		const suffix = search.toString() ? `?${search}` : '';
		const response = await this.fetchImplementation(`${this.websiteUrl}/api/supporters/wall${suffix}`);
		if (!response.ok) {
			throw await NuvioApiError.fromResponse(response);
		}
	...
	private async http<T>(url: string, options: HttpOptions = {}): Promise<T> {
		...
		const response = await this.fetchImplementation(url, { ...init, headers: requestHeaders });
		if (!response.ok) {
			throw await NuvioApiError.fromResponse(response);
		}
