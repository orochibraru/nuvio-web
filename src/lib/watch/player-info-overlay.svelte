		...
		<div class="flex max-h-full w-full max-w-4xl flex-col gap-4 text-white">
			<div class="flex shrink-0 items-center gap-3">
				<span class="flex flex-1 items-center gap-1.5 text-xs font-semibold tracking-[0.2em] text-white/50 uppercase">
					...
					{info.episodeTitle ? subheading || "Episode" : "About this title"}
				</span>
				<Button
					variant="secondary"
					size="icon"
			...
			<div class="flex min-h-0 flex-1 gap-7 overflow-y-auto pr-1 scrollbar-thin md:gap-10">
				{#if poster}
					<img
						src={poster}
						alt=""
			...
			<div class="flex shrink-0 flex-wrap items-center gap-3">
				{#if autoOpened && onResume}
					<Button size="lg" onclick={onResume} class="gap-2 rounded-full font-semibold">
						<PlayIcon data-icon="inline-start" class="size-4 fill-current" />Resume
					</Button>
				{:else}
					<Button size="lg" onclick={onClose} class="gap-2 rounded-full font-semibold">
						<PlayIcon data-icon="inline-start" class="size-4 fill-current" />Keep watching
					</Button>
				{/if}
				<Button variant="secondary" size="lg" href={detailHref} class="rounded-full font-medium">
					Full details
				</Button>
			</div>
		</div>
